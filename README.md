...
- **エディタ内での簡易確認**：自動切り抜き（`extract.py`／rembg）はサーバー側
  （GitHub Actions）でしか実行できないため、編集中のスマホ上では本物の切り抜き結果を
  見ることができません。そこで `color_source`/切り抜き方法が `auto` または
  `auto+brush` のフリーズでは、編集画面に「🔍 切り抜き結果を確認（簡易プレビュー）」
  ボタンが表示されます。押すと、端末上・低解像度（長辺260px程度）でGrabCut風の
  簡易被写体推定（中央矩形を前景シード・外周を確実な背景として初期化し、CIE Lab色空間
  での近さで2値分類→前景平均を再計算して収束するまで数回再分類→3x3多数決でノイズを
  均す。ただし輝度の勾配が大きい＝輪郭らしい画素は多数決の対象から外し、境界がにじまない
  ようにする。本物のGMM＋グラフカットの最適化は行わない）を行い、推定した被写体領域を
  シアン色の半透明オーバーレイで静止フレームに重ねて表示します。**本番のサーバー側切り抜き
  （rembgの本格的なAIモデル）よりずっと粗い目安である**旨を、モーダル上部の目立つバッジと
  本文の両方で明記しています。
  - 採用方式についての補足：OpenCV.jsのGrabCut実装やonnxruntime-webでの軽量rembgモデル
    実行も検討しましたが、いずれも数MB〜数十MBの追加ダウンロードが必要で、
    「外部ライブラリ・ビルド一切なしの単一HTMLファイル」というこのエディタの
    アーキテクチャ上の制約に合わないため、依存ゼロで完結する上記の軽量ヒューリスティックを
    採用しました。実装は `estimateForegroundMaskCoarse`（`index.html` のcore-logic）を参照。
  - 表示中のフリーズに常に追従します：プレビューを開いたまま「完了」してフリーズ編集を
    抜けた場合でも、モーダルは自動的に閉じます。別のフリーズを新たに編集して改めて
    ボタンを押すと、その時点の静止フレームで必ず再計算されるため、古いフリーズの結果を
    誤って見せ続けることはありません。
  - 閉じ方は「✕」ボタン（画面右上に固定表示）・Escキー・モーダル外（背景）タップの
    いずれでも可能です。閉じると編集画面のスクロール・タッチ操作がすぐに復帰します。
  - **メモリ対策**：iPhone実機でページがクラッシュする不具合の対策として、このプレビュー
    まわりで確保するcanvasは常に縮小解像度（解析用は長辺260px、表示用も最大幅720px）に
    留め、フル解像度のImageDataを同時に2枚以上保持しないようにしています。また、
    一時的に確保したcanvas（解析用・オーバーレイ用・プレビュー表示用、いずれも）は
    使い終わり次第 `width`/`height` を0にして即座にバッキングストアを解放します
    （参照を外すだけだとiOS SafariのGCが遅れ、繰り返し開閉するとメモリを使い切って
    ページごと落ちることがあるため）。フリーズ編集モードそのものが保持する静止フレーム
    （`frozenFrameCanvas`）や、「▶ プレビュー」再生用に確保するcanvas群も同様に、
    編集モードを抜ける・再生を止めるタイミングで明示的に解放しています。
...
- タイムライン（着地開始 t=0 として、既定値の場合。以前のバージョンよりゆっくり・
  ...
  4. `0.80-1.50s` **光彩スイープ**：着地完了から`sweep_start_sec`秒後に開始し
