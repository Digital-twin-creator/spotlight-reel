...
```json
{
  ...
  "clips": [
    ...
    },
    {
      ...
      "in": 3.0,
      "out": null,
      "freezes": [ { "time": 1.0, "name": "2本目のフリーズ" } ]
    }
  ],
...
- `clips[].freezes`：そのクリップだけのフリーズ配列。**時刻はそのクリップ自身の
  トリム後0基点タイムライン**で指定する（`in` の値を足す必要はない。例えば `in: 3.0` の
  クリップで `time: 1.0` のフリーズは、トリム後の映像で1.0秒地点＝元動画の4.0秒地点になる）。
