...
- **エディタ内での簡易確認**：自動切り抜き（`extract.py`／rembg）はサーバー側
  ...
  ボタンが表示されます。押すと、端末上・低解像度（長辺200px程度）でGrabCut風の
  簡易被写体推定（中央矩形を前景シード・外周を確実な背景として初期化し、
  それぞれの平均色との近さで2値分類→前景平均を1回だけ更新して再分類→3x3多数決で
  ノイズを均す、という軽量な近似。本物のGMM＋グラフカットの反復最適化は行わない）を行い、
  推定した被写体領域をシアン色の半透明オーバーレイで静止フレームに重ねて表示します。
  **本番のサーバー側切り抜き（rembgの本格的なAIモデル）よりずっと粗い目安である**旨を
  ダイアログ内に明記しています。
