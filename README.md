...
1プロジェクトで複数の動画を1本につなげて出力できます。トップレベルの `video`/`freezes` の
代わりに `clips[]` を指定すると、`render.py` はそちらを正として複数クリップ結合レンダリング
（`render_multi_clip`）を行います。`clips` が無い場合は従来どおり `video`/`freezes` を単一動画
として扱うため、完全に後方互換です。
  ...
  "clips": [
    {
      ...
      "in": 0.0,
      "out": 12.5,
      "freezes": [ { "time": 3.0, "name": "1本目のフリーズ" } ],
      "transition_out": { "type": "crossfade", "sec": 0.5 }
    },
    {
      "video": "b.mov",
      "in": 3.0,
      "out": null,
      "freezes": [ { "time": 4.0, "name": "2本目のフリーズ" } ]
    }
  ],
  "style": { "...": "全クリップ共通（clips使用時もstyle/logo/watermark/hashtagsはプロジェクト全体で1つ）" },
  "logo": { "...": "指定した場合、演出は全体の末尾（最後のクリップの後ろ）に1回だけ付く" },
  "watermark": { "...": "全クリップを通して常時表示（周期の位相もクリップ境界をまたいで連続）" }
}
```

- `clips[].video`：入力動画のパス（JSONファイルからの相対パス、または絶対パス）。
- `clips[].in` / `clips[].out`：そのクリップのトリム範囲（秒）。`in` の既定は `0.0`。
  `out` は省略・`null` で「クリップの最後まで」を表す。
- `clips[].freezes`：そのクリップだけのフリーズ配列。**時刻はそのクリップの元動画
  （トリム前）上の絶対時刻**で指定する（`in`/`out` は再生範囲を絞るメタデータに
  過ぎず、`freezes[].time` はエディタの `<video>.currentTime` と同じ基準＝常に
  元動画の頭からの経過秒数）。上の例で `in: 3.0` のクリップに `time: 4.0` の
  フリーズを指定すると、トリム後の映像では1.0秒地点で静止する（`render.py`の
  `load_project()` が `time - in` を計算し、実際のレンダリングはトリム後0基点の
  時刻で行う）。`in: 0.0` のクリップ（1本目のクリップや、旧`video`/`freezes`
  形式からの後方互換変換）では絶対時刻とトリム後時刻が一致するため見た目の違いは無い。
- `clips[].transition_out`：このクリップから次のクリップへの遷移。
  `type`（`cut`/`fade_black`/`crossfade`/`wipe`。既定 `cut`）と `sec`（重なる秒数。既定 `0.4`。
  `cut` は音声のクリック音を避けるため実際には10msの短いブレンドになる）。
  最後のクリップの `transition_out` は無視される（次のクリップが無いため）。
- `style` / `logo` / `watermark` / `hashtags` / `output` はプロジェクト全体で共有され、
  クリップごとに分かれることはない。ラストロゴは全体の末尾（最後のクリップの後ろ）に1回だけ、
  透かしロゴ・ハッシュタグはクリップ境界をまたいでも表示が途切れず、透かしの明滅・回転の
  周期（位相）も連続する。
- スマホ用エディタ（index.html）では、「① 動画を選ぶ」で1本目を読み込んだ後、
  「🎬 動画を追加」でクリップ一覧が現れ、2本目以降を追加できる。クリップごとに
  サムネイル・尺・トリム（開始/終了）スライダー・並べ替え（↑↓）・削除ができ、
  各カードの「編集」を押すとそのクリップに対して通常どおりシーク・フリーズ編集ができる
  （「動画を作る」は全クリップをまとめて1つのジョブとしてGitHubへ送信する）。
...
- `output.preset`：出力先プリセット。`"instagram_tiktok"`（**既定・省略時**）/
  `"youtube_shorts"` / `"custom"` のいずれか。
  - `"instagram_tiktok"`：常に **1080x1920固定・crf17固定** で出力します（元動画の
    サイズには合わせません）。`output`キー自体を省略した場合もこれと同じ結果になります。
  - `"youtube_shorts"`：**2160x3840** をターゲットにしますが、入力の表示上の解像度が
    それ未満（いわゆる「4K未満」）の場合は、無理にアップスケールせず入力自身の解像度の
    まま出力します。crfは常に**18固定**です。
  - `"custom"`：`output.width`/`output.height`（省略時は1080x1920）・`output.quality`
    をそのまま使います（従来どおりの挙動）。
  - どのプリセットでも `output.width`/`output.height` が明示されている場合は、
    presetの値に関わらずそちらを優先します（`preset`キーが無かった旧JSONとの
    完全な後方互換のため）。
  - `output.fps` を省略した場合は元動画のfpsを使います（presetに関わらず共通）。
  - 入力が出力より高解像度の場合（ダウンスケール）は、`scale`フィルタにLanczos
    アルゴリズムを使い、縮小でボケた輪郭を軽く補償するアンシャープマスク
    （強さ0.3程度）をかけます。
- `output.quality`：`output.preset`が`"custom"`のときだけ参照する、出力のH.264
  エンコード品質。`"standard"`（crf20）/ `"high"`（crf17。**既定・省略時**）/
  `"best"`（crf15）のいずれか。数値が小さいほど高画質・ファイルサイズ大。
...
- `logo.spin`：着地演出（タメ→縮小→セトル→光彩スイープ）が終わった直後に、ロゴの
  水平中心を通る垂直の軸（Y軸）まわりに1回だけ回転させる演出（既定は無効）。
  4隅を3D座標として扱い、Y軸回転＋透視投影（`cv2.warpPerspective`。視点距離＝
  ロゴ幅×2.5）で描画する。90〜270度（裏面側）は実際の裏デザインが無いため、
  表面画像を左右反転したものを表示する。回転が終わった後は、途中で止めた角度
  （`degrees`）のまま静止し続ける（0度へは戻らない）:
  ...
  | キー | 既定値 | 意味 |
  | --- | --- | --- |
  | `enabled` | `false` | 有効にすると着地演出の直後に1回だけ回転する |
  | `sec` | `0.8` | 回転にかける時間（秒） |
  | `degrees` | `360` | 回転角度。`360`＝一回転して正面に戻る。`180`＝裏返った状態で止まる |
  | `ease` | `"in_out"` | 回転速度のイージング。`"in_out"`（滑らかに加減速）または`"linear"`（等速） |
  | `perspective` | `0.4` | 遠近の強さ（0〜1）。`0`は台形変形の無い平面的な幅の縮小のみ、`1`は強い遠近（カードが反り返るような見た目） |
